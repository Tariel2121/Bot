...
keepAlive();
client.login("MTAwMjM2Nzc4MDYzODY5MTQxOQ.GQj9B9.HnQ7vSgpgx-CinLUVaEcfhUOyr17C67lb7uXA0");
...
client.on("channelDelete", (channel) => {
    if (channel.parentID == channel.guild.channels.cache.find((x) => x.name == `${ModMailCategoryName}`).id) {
        ...
        let yembed = new discord.MessageEmbed()
            .setAuthor("MAIL CERRADO!", client.user.displayAvatarURL())
            .setColor(`${CloseEmbedColor}`)
            .setDescription(`${WarningEmoji} ¡No responda a este mensaje hasta que necesite más ayuda!`)
        ...
        if (command == "setup") {
            ...
            if (message.channel.parentID == message.guild.channels.cache.find((x) => x.name == `${ModMailCategoryName}`).id) {
                ...
                let yembed = new discord.MessageEmbed()
                    ...
                    .setColor(`${CloseEmbedColor}`)
                    .addField("Cerrado por:", `${message.author.tag}`)
                    .addField("Server:", ServerName)
                    .addField("Razon:", `\`${args[0] || "[No Reason Provided]"}\``)
            ...
            if (isNaN(args[0]) || !args.length) {
                return message.channel.send({embed: 
                    {color: "RED",
                    description: `${WrongEmoji} ¡Dé la identificación del usuario!`
                    }})
            }
            ...
            let uembed = new discord.MessageEmbed()
                .setAuthor("Correo directo creado:")
